# computation of the T-optimum design \nu = 1, presented in the table 5 in the standard case 
rm(list=ls())
#-----------------------------------------------------------------------------------------------------------------------
#                                                        starting values
#-----------------------------------------------------------------------------------------------------------------------
...
theta1=c(7.2976,4.3860,2.5821,1) #nu=1, competitive model is fixed

theta2.start=c(8.6957,8.0664,12.0566) #non competitive model is fitted
lambda=0   # lambda=0, non-comp fitted
...
while(iter< max.iter){
  ...
  Tp.theta=function(par){
    ...
    eta1=(theta1[1]*xstart[1,])/ ((theta1[2]*(1+(xstart[2,]/theta1[3]))) + xstart[1,]*(1+(((1-theta1[4])*xstart[2,])/theta1[3]))) #10
    eta2=(theta21*xstart[1,])/ ((theta22*(1+(xstart[2,]/theta23)))+xstart[1,]*(1+(((1-lambda)*xstart[2,])/theta23))) #11
  ...
  #                                                        X.HAT
  #-----------------------------------------------------------------------------------------------------------------------
  #delta_1 in T-optimality definition (lack of fit sum of squares when the competitive model is fixed)
